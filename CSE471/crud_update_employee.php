<?php
include '../Controller/crud_functions_employee.php';
...
if (isset($_GET['id'])) {
    if (!empty($_POST)) {
        ...
        // Update the record
        $stmt = $pdo->prepare('UPDATE employee_signed SET id = ?, username = ?, fullname = ?, password = ?, email = ?, department = ? WHERE id = ?');
        $stmt->execute([$id, $username, $fullname, $password, $email, $department, $_GET['id']]);
...
<div class="content update">
    ...
    <form action="crud_update_employee.php?id=<?=$employee['id']?>" method="post">
        ...
        <input type="text" name="fullname" placeholder="Firstname Lastname" value="<?=$employee['fullname']?>" id="fullname">
