<?php
include '../Controller/crud_functions_deliverboy.php';
...
if (isset($_GET['id'])) {
    if (!empty($_POST)) {
        ...
        // Update the record
        $stmt = $pdo->prepare('UPDATE deliveryboy_signed SET id = ?, username = ?, fullname = ?, password = ?, email = ? WHERE id = ?');
        $stmt->execute([$id, $username, $fullname, $password, $email, $_GET['id']]);
...
<div class="content update">
    ...
    <form action="crud_update_deliveryboy.php?id=<?=$deliveryboy['id']?>" method="post">
        ...
        <input type="text" name="fullname" placeholder="Firstname Lastname" value="<?=$deliveryboy['username']?>" id="fullname">
